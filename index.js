...
function createDirectoryContents(templatePath, newProjectPath) {
  ...
  try {
    ...
    process.stdout.write(GREENISH(`Copying template files...\n`))
  } catch (err) {
    console.error(
  ...
  filesToCreate.forEach(file => {
    ...
    if (stats.isFile()) {
      ...
      try {
        ...
        console.log(GREENISH(' -> ') + BLUEISH(writePath) + ' file created.\n')
      } catch (err) {
        console.error(
      ...
      try {
        ...
        console.log(
          GREENISH(' -> ') + BLUEISH(newProjectPath) + ' directory created.\n'
        )
      } catch (err) {
        console.error(
